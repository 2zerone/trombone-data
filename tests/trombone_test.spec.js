...
test('TROMBONE 티켓 사전 데이터 등록 시작', async ({ page }) => {
  ...
  // 8. 워크플로우 컴포넌트 등록
  await workflowComponentManager.createComponent(`${config.project.code}-결재컴포넌트(결재자)`, 1, '결재자', 4);
  await workflowComponentManager.createComponent(`${config.project.code}-파이프라인-STG`, 2, 'STG', 3);
  ...
  await workflowComponentManager.createComponent(`${config.project.code}-코드병합-STG`, 6, 'STG', 3);
  await workflowComponentManager.createComponent(`${config.project.code}-코드병합-PRD`, 6, 'PRD', 3);
  await workflowComponentManager.createComponent(`${config.project.code}-통합테스트`, 8);
  await workflowComponentManager.createComponent(`${config.project.code}-예약배포PRD`, 10, 'PRD', 3);
}); 